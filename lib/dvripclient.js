...
class DVRIPClient extends EventEmitter {
	...
	setSessionId(newSessionId = NullUint32) {
		...
		if(newSessionId instanceof Buffer) {
			...
			newSess = {buffer: Buffer.from(newSessionId.substr(2) /*Skip "0x"*/, "hex"), string: newSessionId};
	...
	buildMessage({Command, MessageBody}) {
		...
		const fromBuffer = [
			...
			cmdSeq >> 24, cmdSeq >> 16, cmdSeq >> 8, cmdSeq, //Sequence ID as Big Endian
			0, 0, 0, 0, 0, //Reserved?
			Command, Command >> 8,
	...
	async login({Username = "admin", Password = ""}) {
		...
		/*let msg_md5 = Buffer.from(md5(Password), "hex");
		let PassWord = "";

		for(let i = 0; i < 8; i++) {
			let n = (msg_md5[2 * i] + msg_md5[2 * i + 1]) % 62;
			if (n > 9)
				n += (n > 35) ? 13 : 7;

			PassWord += String.fromCharCode(n + 48);
		}*/
	 ...
	 */
	async getVideoStream({StreamType = "Main", Channel = 0, CombinMode = "NONE"}) {
		if(this._streamClient)
